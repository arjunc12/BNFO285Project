...
data <- data[,nums]
data$resp.simple <- data0$resp.simple
...
score0 <- read.csv("scoringData-release.csv")
score <- score0
score$Patiend_id <- NULL
nums2 <- sapply(score, is.numeric)
score <- score[,nums2]
...
remission_test2 = predict(remission_fit2, test, OOB=T)
# 
...
response_test2_ein = error_rate(test$resp.simple, response_test2)
